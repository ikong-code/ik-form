...
const ColorPicker = ({ defaultValue = '#000000', value, onChange }: IProps) => {
  ...
  const getColor = tinycolor(value || defaultValue).toRgb()
  ...
  const setColor = (rgba: ColorFormats.RGBA) => {
    ...
    const colorStr = getHexColor(rgbaString)
    onChange(colorStr)
  }
    ...
    <div className="color-picker">
      ...
      <div className="picker-gradient-presets">
        {presetColors.map((col, index) => (
          <div className="picker-gradient-col" v-for="(col, index) in presetColors" key={index}>
            {col.map(c => (
              <div
